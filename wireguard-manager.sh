...
function installing-system-requirements() {
  if { [ "${DISTRO}" == "ubuntu" ] || [ "${DISTRO}" == "debian" ] || [ "${DISTRO}" == "raspbian" ] || [ "${DISTRO}" == "pop" ] || [ "${DISTRO}" == "kali" ] || [ "${DISTRO}" == "linuxmint" ] || [ "${DISTRO}" == "fedora" ] || [ "${DISTRO}" == "centos" ] || [ "${DISTRO}" == "rhel" ] || [ "${DISTRO}" == "arch" ] || [ "${DISTRO}" == "archarm" ] || [ "${DISTRO}" == "manjaro" ] || [ "${DISTRO}" == "alpine" ] || [ "${DISTRO}" == "freebsd" ] || [ "${DISTRO}" == "neon" ] || [ "${DISTRO}" == "almalinux" ] || [ "${DISTRO}" == "rocky" ] || [ "${DISTRO}" == "ol" ]; }; then
    if { [ ! -x "$(command -v curl)" ] || [ ! -x "$(command -v iptables)" ] || [ ! -x "$(command -v bc)" ] || [ ! -x "$(command -v jq)" ] || [ ! -x "$(command -v cron)" ] || [ ! -x "$(command -v sed)" ] || [ ! -x "$(command -v zip)" ] || [ ! -x "$(command -v unzip)" ] || [ ! -x "$(command -v grep)" ] || [ ! -x "$(command -v awk)" ] || [ ! -x "$(command -v shuf)" ] || [ ! -x "$(command -v openssl)" ] || [ ! -x "$(command -v ntpd)" ] || [ ! -x "$(command -v lsof)" ]; }; then
      ...
      elif { [ "${DISTRO}" == "fedora" ] || [ "${DISTRO}" == "centos" ] || [ "${DISTRO}" == "rhel" ] || [ "${DISTRO}" == "almalinux" ] || [ "${DISTRO}" == "rocky" ] || [ "${DISTRO}" == "ol" ]; }; then
  ...
  function install-kernel-headers() {
    if { [ -f "${WIREGUARD_INTERFACE}" ] || [ -f "${WIREGUARD_PEER}" ]; }; then
      ...
      if (($(echo "${LINUX_HEADER_KERNEL_CURRENT_VERSION} <= ${LINUX_HEADER_KERNEL_VERSION_LIMIT}" | bc -l))); then
        ...
        elif { [ "${DISTRO}" == "fedora" ] || [ "${DISTRO}" == "ol" ]; }; then
          dnf update -y
  ...
  function install-wireguard-server() {
    if { [ ! -x "$(command -v wg)" ] || [ ! -x "$(command -v qrencode)" ]; }; then
      if { [ -f "${WIREGUARD_INTERFACE}" ] || [ -f "${WIREGUARD_PEER}" ]; }; then
        ...
        elif { [ "${DISTRO}" == "almalinux" ] || [ "${DISTRO}" == "rocky" ] || [ "${DISTRO}" == "ol" ]; }; then
          yum update -y
          yum install elrepo-release epel-release -y
          yum install kmod-wireguard wireguard-tools qrencode haveged -y
        fi
      ...
      if [ "${INSTALL_UNBOUND}" = "y" ]; then
        if [ ! -x "$(command -v unbound)" ]; then
          if [ "${DISTRO}" == "ubuntu" ]; then
            ...
            apt-get install unbound unbound-host e2fsprogs -y
          elif { [ "${DISTRO}" == "centos" ] || [ "${DISTRO}" == "rhel" ] || [ "${DISTRO}" == "almalinux" ] || [ "${DISTRO}" == "rocky" ] || [ "${DISTRO}" == "ol" ]; }; then
      ...
      8) # Uninstall WireGuard and purging files
        if { [ -f "${WIREGUARD_INTERFACE}" ] || [ -f "${WIREGUARD_PEER}" ]; }; then
          if [ -x "$(command -v wg)" ]; then
            ...
            if { [ "${DISTRO}" == "centos" ] || [ "${DISTRO}" == "almalinux" ] || [ "${DISTRO}" == "rocky" ] || [ "${DISTRO}" == "ol" ]; }; then
        ...
        if [ -f "${UNBOUND_MANAGER}" ]; then
          if [ -x "$(command -v unbound)" ]; then
            ...
            elif { [ "${DISTRO}" == "arch" ] || [ "${DISTRO}" == "archarm" ] || [ "${DISTRO}" == "manjaro" ] || [ "${DISTRO}" == "almalinux" ] || [ "${DISTRO}" == "rocky" ] || [ "${DISTRO}" == "ol" ]; }; then
